...
{{- else if .Values.pulsarv4.enabled }}
...
pulsar:
{{- if contains .Values.pulsarv4.name .Release.Name }}
  address: {{ .Release.Name }}-proxy
{{- else }}
  address: {{ .Release.Name }}-{{ .Values.pulsarv4.name }}-proxy
{{- end }}
  port: {{ .Values.pulsarv4.proxy.ports.pulsar }}
  maxMessageSize: {{ .Values.pulsarv4.broker.configData.maxMessageSize }}
{{- end }}
